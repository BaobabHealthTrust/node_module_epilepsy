...
function setNextButtonText(str){
    __$('nextButton').innerHTML = "<span>" + str + "</span>";
}
...
function addOnMouseDownAction(optionText){
    var choices = __$('options').getElementsByTagName('li');
...
function onYearUpdateDateTime(main_field_str, year_str, modifier_str){
    ...
    estimated_date = formatDate(__$(main_field_str).value, __$(year_str).value,0);


    if((this_year - __$(year_str).value > 2))  {

        estimated_date = formatDate(__$(year_str).value, 'unknown', 1);
    }
    __$(main_field_str).value = estimated_date['value_datetime'];
    __$(modifier_str).value   = estimated_date['value_modifier'];
}

function onMonthUpdateDateTime(main_field_str, month_str, modifier_str){

    var estimated_date = formatDate(__$(main_field_str).value, __$(month_str).value, 1);

    __$(main_field_str).value = estimated_date['value_datetime'];
    __$(modifier_str).value   = estimated_date['value_modifier'];

    if((__$(month_str).value.toLowerCase() != 'unknown'))  {
        estimated_date = formatDate(__$(main_field_str).value, 'unknown', 2);
